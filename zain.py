            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sRANA-MZ{CP}%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                        self.cp.append("%s•%s•%s%s%s"%(fl.get("id"),i,d,m,y))
                        open("RANAMZ-CP/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y))
                        break
                    except(KeyError, IOError):
                        m = " "
                        d = " "
                    ...
                    print("\r%s[%sRANA-MZ{CP}%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("RANAMZ-CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sRANA-MZ{OK}%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("RANAMZ-OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                else:continue
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sRANA-MZ{OK}%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sRANA-MZ{CP}%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
                    ...
                    h_cp = "\r%s[%sRANA-MZ{CP}%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("RANAMZ-CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
...
def cek_hasil():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sExample File : RANAMZ-CP/%s.txt'%(O,P,O,P,tanggal))
...
def var_author():
    ...
    mlaku('%s║     • %sRai  ZAIN%(O,P))
    mlaku('%s║     • %s MPG MAKER%(O,P))
    mlaku('%s║'%(O))
...
def started():
    ...
    print('%s╠══[%s•%s] %sAccount [OK] Saved To RANAMZ-OK/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %sAccount [CP] Saved To RANAMZ-CP/%s.txt'%(O,P,O,P,tanggal))
